...
const Problem: React.FC = () => {
  ...
  return (
    <section className="py-32 bg-brandBlack text-white overflow-hidden border-t border-white/5" id="problem">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-12">
          {problems.map((problem, idx) => (
            <div 
              key={idx} 
              className="glass-light p-10 rounded-[3rem] border border-white/5 hover:border-white/20 transition-all duration-500 reveal group"
            ...
            >
              ...
              <p className="text-gray-400 leading-relaxed font-medium">{problem.text}</p>
            </div>
          ))}
        </div>
      </div>
